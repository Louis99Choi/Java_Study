...
public class Lab2 {
	...
	public static double calculateDewPointTemperature(double TC, double RH) {
		...
		double DewPoint = (243.12 * (Math.log(RH / 100) + k)) / (17.62 - (Math.log(RH / 100) + k));
	...
	/*
	 * 배열과 for문을 이용해 섭씨온도와 상대습도를 행과 열로 하는 간단한 이슬점 표를 출력하며 입력값 섭씨온도와 상대습도를 정규화시켜
	 * 매개변수로 받고 그 값을 통해 표에서 입력값과 근사한 값을 표시하는 매서드
	 */
	public static void printDewPointTemperatureTable(int stdTC, int stdRH) {
		...
		for (int i = 0; i < rhList.length; i++) {
			if (i == stdRHindex)
				...
				System.out.printf("%d%s", rhList[i], tab);
		}
		...
		for (int i = 0; i < tcList.length; i++) {
			...
			for (int k = 0; k < rhList.length; k++) {
				if (Math.round(calculateDewPointTemperature(tcList[i], rhList[k])) < 0)
					break;
	...
	//입력값을 가장 근사한 5의 배수로 정규화해 인자로 받아 배열과 for문을 이용해 체감온도표를 출력하는 매서드.
	public static void printWindChillTemperatureTable(int stdTF, int stdV) {
		...
		for (int i = 0; i < tfList.length; i++) {
			if (i == stdTFindex)
				System.out.printf("%d%s", tfList[i], starIndex);
			else
				System.out.printf("%d%s", tfList[i], tab);
		}
			...
			for (int k = 0; k < tfList.length; k++) {
				if ((k == stdTFindex && i <= stdVindex) || (i == stdVindex && k < stdTFindex))
					System.out.printf("%d%s", 
							Math.round(calculateWindChillTemperature(tfList[k], vList[i])), starIndex);

				else
					System.out.printf("%d%s", 
							Math.round(calculateWindChillTemperature(tfList[k], vList[i])), tab);
			}
		}

		System.out.printf("\n");
		for (int i = 0; i < 158; i++)
			System.out.printf("="); // WindChill Table boundary
	}

	/*
	 * 프로그램 실행 중 1을 입력해 이슬점 메뉴를 선택할 경우 실행되는 매서드로 공식에 필요한 섭씨온도와 상대습도를 입력받아 calculate_
	 * 매서드로 전달하고 구한 이슬점을 출력하는 매서드
	 */
	...
	/*
	 * 프로그램 실행 중 2를 입력해 체감온도 메뉴를 선택할 경우 실행되는 매서드로 공식에 필요한 화씨온도와 풍속 입력받아 calculate_
	 * 매서드로 전달하고 구한 체감온도를 출력하는 매서드
	 */
	public static void selectedWindChill() {
		...
		System.out.println("-----------------------------------------------------------------------------------------");
		System.out.printf("Input 'Temperature'(°F) and 'Vector of wind' to calculate WInd Chill Temperature: "); 
		...
		// 이슬점을 구하는 매소드에 인자로 inputTF와 inputV를 넣고 얻은 체감온도를 outWindChill 변수에 저장.
		outWindChill = calculateWindChillTemperature(inputTF, inputV); 
		
		// 입력을 통해 구한 체감온도를 반올림해서 소수점 첫째 자리까지 출력.
		System.out.println("Wind Chill Temperature : " + Math.round(outWindChill * 10) / 10.0); 

		//체감온도표를 출력하는 매서드 실행
		printWindChillTemperatureTable((int) Math.round(inputTF - inputTF % 5),
				(int) Math.round(inputV - inputV % 5 + 5));
	...
	public static void showResponseAdvice(double wcTemperature) {
		...
		int index = (wcTemperature >= -3.2) ? 0
				: (wcTemperature >= -10.5) ? 1 : (wcTemperature >= -15.4) ? 2 : (wcTemperature < -15.4) ? 3 : 3;
		System.out.println("\n<외출 시 주의 및 권고사항>");
	...
	/*
	 * 각각 이슬점과 체감온도를 구하는 메뉴 안내문을 출력하고 UserInput.getIntegerBetween 매서드를 통해 메뉴를 선택한 후
	 * switch문을 이용해 해당 메뉴(selectedDewPoint 또는 selectedWindChill) 매서드를 실행.
	 */
